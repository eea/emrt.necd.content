...
class IObservation(model.Schema, IImageScaleTraversable):
    ...
    directives.widget("scenario", CheckBoxFieldWidget)
    scenario = schema.List(
        title="Scenario Type",
        value_type=schema.Choice(vocabulary="emrt.necd.content.scenario_type"),
        required=False,
    )
    ...
    directives.widget("activity_data", CheckBoxFieldWidget)
...
class Observation(Container):
    ...
    def activity_data_value(self):
        return get_list_from_vocab(
            self.aq_parent,
            "emrt.necd.content.activity_data",
            self.activity_data,
        )
    ...
    def wf_status(self):
        if self.get_status() in ["draft"]:
            ...
            return [
                "Counterparts comments requested",
                StatusIcon.CONCLUSION.value,
            ]
        else:
            questions = self.get_values_cat("Question")
            if questions:
    ...
    def observation_status(self):
        ...
        if status in ["draft", "counterpart-comments", "observation-draft"]:
            return "SE"
    ...
    def myHistory(self):
        ...
        for item in observation_history:
            ...
            if i_rstate == "draft":
                ...
                i_rstate in ("conclusions", "conclusions-lr-denied")
...
def set_form_widgets(form_instance):
    ...
    form_instance.groups = [
        g for g in form_instance.groups if g.label == "label_schema_default"
    ]
...
class ObservationView(ObservationMixin):
    def get_current_counterparters(self):
        ...
        users = [u[0] for u in local_roles if ROLE_CP in u[1]]
...
class EditConclusionP2AndCloseComments(BrowserView):
    ...
    def render(self):
        ...
        api.content.transition(obj=self.context, transition="finish-comments")
        conclusions = self.context.get_values_cat("Conclusions")
